...
const ShopCard = ({_id, seller, selleremail, toyname, category, price, quantity, rating, img}) => {
    return (
        <>
           <div className="flex-shrink-0   relative  overflow-hidden bg-[#406147] rounded-lg  shadow-lg">
  <svg
  ...
  <div className="relative text-white px-6 pb-6 mt-6">
    <span className="block opacity-75 -mb-1 text-sm md:text-base uppercase">{category}</span>
    <div className="flex justify-between">
      <span className="block font-semibold md:text-xl truncate">{toyname} </span>
      <span className="block bg-white rounded-full text-orange-500 text-xs font-bold px-3 py-2 leading-none flex items-center">
        ${price}
      </span>
    </div>
<div><span className='text-sm'>quantity : {quantity}</span> </div>
...
<span className='text-white font-semibold text-am'>{rating}</span>
        </div>
        <div class="flex items-center gap-1">
          
  <Link to={`/toy/${_id}`} class="text-sm  text-white border border-white px-2 rounded-lg hover:bg-[#406147]">
